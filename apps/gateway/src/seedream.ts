...
export class SeedreamGatewayError extends Error {
  readonly code: SeedreamErrorCode;
  readonly status: number;

  constructor(
    code: SeedreamErrorCode,
    message: string,
    status: number,
  ) {
    super(message);
    this.name = "SeedreamGatewayError";
    this.code = code;
    this.status = status;
  }
}
...
export async function generateSeedreamAvatar(
  ...
  if (!options.apiKey.trim()) {
    throw new SeedreamGatewayError(
      "SEEDREAM_MISSING_CONFIG",
  ...
  if (generated?.b64_json) {
    return { data: [{ b64_json: generated.b64_json }], model };
  }
  ...
  const b64Json = Buffer.from(
    await readLimitedBytes(imageResponse, MAX_GENERATED_IMAGE_BYTES),
  ).toString("base64");
...
async function readJsonResponse<T>(response: Response, limit: number): Promise<T | null> {
  try {
    ...
    if (error instanceof SeedreamGatewayError) throw error;
    return null;
  }
}
...
function invalidResponseSize(): SeedreamGatewayError {
  return new SeedreamGatewayError(
    "SEEDREAM_INVALID_RESPONSE",
