...
if (-not $transportPeerFound.Success -or
    ...
    $transportPeerLost.Value -notmatch 'PF_STATE_PEER_FOUND[\s\S]*PF_STATE_WAITING_CONFIRM[\s\S]*pf_start_pairing_cooldown\(\)[\s\S]*sg_pairing_peer_pending\s*=\s*false[\s\S]*pf_dispatch\(PF_EVENT_RESET\)') {
    throw 'Sustained peer loss must leave pairing and suppress immediate rematching'
}
